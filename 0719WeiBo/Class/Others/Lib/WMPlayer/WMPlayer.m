...
//        [_closeBtn addTarget:self action:@selector(closeTheVideo:) forControlEvents:UIControlEventTouchUpInside];
...
//                make.height.mas_equalTo(40);
//                make.top.mas_equalTo(self.center.y + 90);
...
- (void)finishedPlay:(NSTimer *)timer{
    if (self.currentTime == self.duration&&self.player.rate==.0f) {
        ...
        //播放完成后的通知
        [[NSNotificationCenter defaultCenter] postNotificationName:@"finishedPlay" object:self];
        [self.durationTimer invalidate];
...
-(void)initTimer{
    ...
    _timeObserver = [self.player addPeriodicTimeObserverForInterval:CMTimeMakeWithSeconds(interval, NSEC_PER_SEC)  queue:NULL /* If you pass NULL, the main queue is used. */ usingBlock:^(CMTime time){
